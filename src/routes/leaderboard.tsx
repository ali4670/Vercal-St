    ...
    <div className="min-h-screen bg-[#030303] text-white p-10 pt-24">
      ...
      <div className="space-y-4">
        {students.map((student, i) => (
          <div
            key={student.id}
            className="bg-white/5 p-4 rounded-2xl flex items-center gap-4"
          >
            <span className="text-xl font-black text-[#CCFF00]">#{i + 1}</span>
            <span className="font-bold flex-1">{student.username}</span>
            <span className="font-black text-[#CCFF00]">{student.xp} XP</span>
          </div>
        ))}
      </div>
    </div>
